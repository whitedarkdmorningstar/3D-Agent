...
export async function fetchInvoicesOfACustomerAsync(
  ...
  const invoices = await db.getAllAsync<InvoiceOutput>(
    `SELECT * FROM invoices WHERE name = ? AND trashed = ? AND week = ? AND year = ? ORDER BY ${options.orderBy} ${options.order} LIMIT ? OFFSET ?`,
    [
      name,
      options.trashed,
      options.week,
